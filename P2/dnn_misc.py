...
class linear_layer:
    ...
    def forward(self, X):
        ...
        N,D=X.shape
        I_b=np.ones((N,1))
        XW=np.matmul(X,self.params['W'])
        Ib=np.matmul(I_b,self.params['b'])
        forward_output=np.add(XW,Ib)
...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output=np.absolute(X)
        forward_output=np.add(X,forward_output)
        forward_output=np.divide(forward_output,2.0)
        self.mask=(X>np.zeros(X.shape))
