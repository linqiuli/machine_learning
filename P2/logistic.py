...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    sig_array=np.vectorize(sigmoid)

    for i in range(0,max_iterations):
        XW_Y=np.matmul(X,w)+b*np.ones(N)
        intervalue=sig_array(XW_Y)
        delta_b=intervalue-y
        delta_w=np.matmul(delta_b.transpose(),X)
        
        b=b-step_size*np.sum(delta_b,axis=0)/float(N)
        w=w-step_size*delta_w/float(N)
...
def binary_predict(X, w, b):
    ...
    preds=np.add(np.matmul(X,w),b*v_I)
    sig_array=np.vectorize(sigmoid)
    prediction=sig_array(preds)
    threshhold=np.ones(N)/2.0
    preds=(prediction >= threshhold)
...
def multinomial_train(X, y, C, 
    ...
    while(i<max_iterations):
        inter_y0=np.matmul(X,w.transpose())
        v_I=np.ones((N,1))
        inter_y1=inter_y0+v_I*b
        inter_y2=np.exp(inter_y1)
        inter_sum=1/np.sum(inter_y2,axis=1)
        inter_y3=(np.ones((C,1))*inter_sum).transpose()
        inter_y4=np.multiply(inter_y2,inter_y3)
        delta_b=np.sum(inter_y4-y_tmp,axis=0)
        delta_w=np.matmul(np.transpose(inter_y4-y_tmp),X)
        w=w-step_size*delta_w/float(N)
        b=b-step_size*delta_b/float(N)
        i=i+1
...
def OVR_predict(X, w, b):
    ...
    XW=np.matmul(X,np.transpose(w))
    d=v_I.reshape(N,1)*b.reshape(1,C)
    y=np.add(XW,d)
    for i in range(0,N):
        preds[i]=np.argmax(y[i])
