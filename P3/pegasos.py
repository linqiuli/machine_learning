...
def objective_function(X, y, w, lamb):
    ...
    obj_value=lamb/2*np.sum(w*w)
    y=1.0-np.multiply(np.matmul(X,w).transpose(),y)
    y=np.maximum(0.0,y)
    sum0=float(np.sum(y))/float(len(y))
    obj_value=obj_value+sum0
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        wt=(1.0-1.0/float(iter))*w
        for i in A_t:
            if (ytrain[i]*np.dot(Xtrain[i],w) <1):
                yx=ytrain[i]*Xtrain[i]
                yx=yx.reshape(-1,1)
                wt+=nt/float(k)*yx
        tmp=min(1.0,1.0/np.sqrt(lamb*np.sum(wt*wt)))
        w=tmp*wt
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    for i in range(0,n):
        if(prediction[i]*ytest[i]>=0):
            correct+=1
    test_acc=float(correct)/float(n)
