...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  for i in lambds:
    ...
    y_predict=np.matmul(np.transpose(w),np.transpose(Xval))
    for j in range(0,len(yval)):
      new_error+=(yval[j]-y_predict[j])**2
    if (new_error <error):
        error=new_error
        bestlambda=i
  #####################################################		 
...
def test_error(w, X, y):
  """
    ...
    """
  y_predict=np.matmul(np.transpose(w),np.transpose(X))
  err=0.0
  for i in range(0,len(y)):
    err+=(y[i]-y_predict[i])**2
  return err/len(y)
