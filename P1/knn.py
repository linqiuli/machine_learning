...
def compute_distances(Xtrain, X):
	...
	for i in range(0,num_test):
		for j in range(0,num_train):
				#dists[i][j]+=(X[i][k]-Xtrain[j][k])**2
				difference=np.subtract(X[i],Xtrain[j])**2
				dists[i][j]=np.sqrt(np.sum(difference))
        #####################################################		 
	return dists
...
def predict_labels(k, ytrain, dists):
	...
	labels=np.zeros(k)
	labels_index=np.zeros(k,dtype=int)
	index_y=np.zeros(k)
	ypred=np.zeros((num_test,),dtype=int)
	for i in range(0,num_test):
		for j in range(0,k):
			k_near=np.argmin(dists[i])
			labels_index[j]=k_near
			labels[j]=ytrain[k_near]
			index_y[j]=dists[i][k_near]
			dists[i][k_near]=np.inf
		for l in range(0,k):
			ind=labels_index[l]
			dists[i][ind]=index_y[l]
		unique,counts=np.unique(labels,return_counts=True)
		index=np.argmax(counts)
		ypred[i]=unique[index]
	#####################################################
	return ypred
